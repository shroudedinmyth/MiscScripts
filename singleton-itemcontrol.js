...
var ItemControl = {
	...
	isSecondaryEquipmentAllowed: function(secList, eqpmnt) {
		/**
			equippableSecondaries: {
				smallShield: true,
				
		*/
		var i;
		var keys = Object.keys(secList);
		for (i in keys) {
			if (eqpmnt.getCustomKeyword() === secList[i]) {
				break;
			}
		}
		if (i === count) {
	...
	getEquippedWeapon: function(unit) {
		...
		if (!unit.custom.equipment.primary) {
			return null;
		}
	...
	getEquippedSecondary: function(unit) {
		if (unit === null) {
			return null;
		}
		
		if (!unit.custom.equipment.secondary) {
			return null;
		}
	...
	//should change to add an E marker at bottom right hand corner of icon when equipped
	setEquippedWeapon: function(unit, targetItem) {
		var i, item;
		var count = UnitItemControl.getPossessionItemCount(unit);
		var fastIndex = -1, targetIndex = -1;
		var equipment = UnitItemControl.getEquipment(unit);
		var equippedWeapon = equipment.primary;
		var equippedSecondary = equipment.secondary;
		
		if (targetItem.getWeaponCategoryType() === WeaponCategoryType.MAGIC || targetItem.getCustomKeyword() === "Magic") {
			if (ItemControl.isShield(equippedSecondary)) {
				equipment.secondary = null;
				equipment.primary = targetItem;
			}
		}
		
		else if (targetItem.getWeaponCategoryType() === WeaponCategoryType.PHYSICS) {
			var wTypeName = equippedWeapon.getWeaponType().name;
			if (ItemControl.isRangedWeapon(targetItem)) {
				if (!ItemControl.isMatchingAmmo(targetItem, equippedSecondary)) {
					equipment.secondary = null;
					equipment.primary = targetItem;
				}
				else {
	...
	setEquippedSecondary: function(unit, targetItem) {
		var equipment = UnitItemControl.getEquipment();
		var equippedWeapon = equipment.primary;
		var equippedSecondary = equipment.secondary;
		
		if (ItemControl.isAmmunition(targetItem)) {
			if (ItemControl.isMatchingAmmo(equippedWeapon, targetItem)) {
				equipment.secondary = targetItem;
			}
			else return;
		}
		
		else if (ItemControl.isShield(targetItem)) {
			if (ItemControl.isRangedWeapon(equippedWeapon) || ItemControl.isMagic(equippedWeapon)) {
				return;
			}
			else {
				equipment.secondary = targetItem;
		...
		else if (targetItem.getCustomKeyword() === "Accessory") {
			equipment.secondary = targetItem;
		}
	...
	//Check if the unit CAN equip the secondary item.
	isSecondaryAvailable: function(unit, item) {
		if (item === null) {
			return false;
		}
		...
		if (!this.isSecondaryEquipmentAllowed(unit.getClass().custom.secondaryWeaponTypes, item) {
			return false;
		}
		...
		if (ItemControl.isShield(item)) {
			//check if "Weapon" is prohibited
			if (StateControl.isBadStateFlag(unit, BadStateFlag.PHYSICS)) {
				return false;
			}
			//Check "Weapon Level" (update later to replace with weapon rank once implemented)
		}
	...
	isMatchingAmmo: function(weapon, sec) {
		var wType = weapon.getWeaponType();
		var wTypeName = wType.getName();
		var secType = sec.getItemType();
		var match = false;
		if (!secType === ItemType.CUSTOM || !ItemControl.isAmmunition(sec)) {
			return match;
		}
		else if ((wTypeName === "Bow" || wTypeName === "Crossbow") && sec.getCustomKeyword() === "Arrow") {
			match = true;
		}
		else if (wTypeName === "Gun" && sec.getCustomKeyword() === "Bullet") {
			match = true;
		}
		else if (wTypeName === "Ballista" && sec.getCustomKeyword() === "Ammo") {
			match = true;
		}
	...
	isAmmunition: function(item) {
		if (item.getItemType() === ItemType.CUSTOM) {
			var kword = item.getCustomKeyword();
			return kword === "Arrow" || kword === "Bullet" || kword === "Ammo";
		}
		return false;
	},
	
	isShield: function(item) {
		if (item.getItemType() === ItemType.CUSTOM) {
			var kword = item.getCustomKeyword();
			return kword === "SShield" || kword === "MShield" || kword === "LShield"
		}
		return false;
	},
	
	isRangedWeapon: function(weapon) {
		if (weapon.isWeapon()) {
			var rType = weapon.getWeaponType().getName();
			return (rType === "Bow" || rType === "Crossbow" || rType === "Gun" || rType === "Ballista");
		}
		return false;
	},
	
	isMagic: function(item) {
		return item.getWeaponType() === WeaponType.MAGIC || item.isWand();
	}
};
...
var UnitItemControl = {
	...
	//equipment is an object with two slots: primary and secondary
	getEquipment: function(unit) {
		return unit.custom.equipment;
	},
	
	setEquipment: function(unit, item) {
		if (item.custom.equipType === 'primary') {
			if (ItemControl.isWeaponAvailable(item)) {
				unit.custom.equipment.primary = item;
			}
		}
		else if (item.custom.equipType === 'secondary') {
			if (ItemControl.isSecondaryAvailable(item) {
				unit.custom.equipment.secondary = item;
			}
		}
	},
